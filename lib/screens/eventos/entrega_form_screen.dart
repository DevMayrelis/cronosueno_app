// lib/screens/events/entrega_form_screen.dart
...
class EntregaFormScreen extends StatefulWidget {
  ...
  const EntregaFormScreen({super.key, this.onEventoGuardado});
...
class _EntregaFormScreenState extends State<EntregaFormScreen> {
  ...
  void dispose() {
    ...
    _tipoEntregaController.dispose();
  ...
  Future<void> _guardarEntrega() async {
    ...
    if (_materiaController.text.isEmpty ||
        _tipoEntregaController.text.isEmpty ||
        _selectedDate == null ||
        _selectedTime == null) {
    ...
    try {
      ...
      final evento = EventoAcademico(
        tipo: 'Entrega',
        titulo: _tipoEntregaController.text,
      ...
      // DESCRIPCIÓN MEJORADA DEL EVENTO - CON EMOJIS Y FORMATO MEJORADO
      final descripcionEvento = '${evento.titulo} - ${evento.materia}\n'
          ...
          '${_lugarController.text.isNotEmpty ? _lugarController.text : "Método de entrega por definir"}';
      ...
      if (!mounted) return;
      _mostrarSnackBar('Error al guardar la entrega: $e');
    } finally {
      if (mounted) {
                ...
                child: Column(
                  ...
                  children: [
                    _buildTextField(
                      ...
                      label: 'Materia *',
                      hintText: 'Ej: Programación, Diseño',
                    ),
                    const SizedBox(height: 20),
                    _buildTextField(
                      controller: _tipoEntregaController,
                      label: 'Tipo de entrega *',
                      hintText: 'Ej: Proyecto, Tarea, Trabajo',
                    ),
                    const SizedBox(height: 20),
                    _buildTextField(
                    ...
                    _buildTextField(
                      ...
                      label: 'Lugar/Método de entrega',
                      hintText: 'Ej: Plataforma virtual, Oficina',
                    ),
                    const SizedBox(height: 20),
                    _buildDateSelector(),
                    const Divider(),
    ...
    return Column(
      ...
      children: [
        SwitchListTile(
          ...
          subtitle: Text(
            'Recibir notificación antes de la entrega',
  ...
  Widget _buildSaveButton() {
    return SizedBox(
      ...
      height: 56,
      child: FilledButton(
        onPressed: _guardando ? null : _guardarEntrega,
